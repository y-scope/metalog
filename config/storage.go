...
type StorageBackendConfig struct {
	Type           string `yaml:"type"`     // "s3" (default), "http", "fs"
	Bucket         string `yaml:"bucket"`   // default bucket for this backend
	BaseURL        string `yaml:"baseUrl"`  // for http backend
	BasePath       string `yaml:"basePath"` // unused by current backends; fs uses bucket as root dir
