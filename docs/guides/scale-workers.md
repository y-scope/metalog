...
- **In-process workers (`WorkerUnit`)** — run as threads inside the Node JVM. A single `TaskPrefetcher` thread batch-claims tasks from any table into a shared `LinkedBlockingQueue`; worker threads call `queue.take()` instead of hitting the database directly. Used for development and testing.
- **Standalone workers (`Worker`)** — run as separate JVM processes on dedicated machines. Each worker thread claims tasks directly using `SELECT ... FOR UPDATE` with per-thread exponential backoff (no prefetch queue). Claims are scoped to a single configured table. Used in production for fault isolation and independent scaling.

## Overview

| Component | Role |
|-----------|------|
| **Coordinator** | 4 per-table threads + Node-level threads (BatchingWriter, watchdog, HA, reconciliation) |
| **Workers (in-process)** | Threads inside the Node JVM, sharing resources (dev/test) |
| **Workers (standalone)** | Separate JVM processes on dedicated machines (production) |
...
| Standalone mode | Fault isolation, independent scaling, no GC interference (production) |
| Prefetch queue (in-process) | One DB claim transaction per batch instead of one per worker poll; workers wake via `queue.take()` at zero CPU cost |
| Direct claiming (standalone) | Each worker thread claims independently; suitable for isolated JVM processes scoped to one table |
...
1. **Task Creation** (Planner Thread)
   - Queries database for files in `IR_ARCHIVE_CONSOLIDATION_PENDING` state
   - Groups files by `application_id` using SparkJobPolicy
   - Creates `TaskPayload` with IR paths and archive path
   - Inserts to `_task_queue` table with state = `pending`

2. **Task Claiming**
   - **In-process (`WorkerUnit`):** A single `TaskPrefetcher` thread executes `SELECT ... FOR UPDATE` + `UPDATE` in a READ COMMITTED transaction to batch-claim up to `prefetchQueueSize` tasks from any table. Worker threads call `queue.take()` (blocking indefinitely). Prefetcher backs off (1 s → 32 s) when no tasks are available.
   - **Standalone (`Worker`):** Each worker thread independently executes `SELECT ... FOR UPDATE` + `UPDATE` to claim one task at a time from the configured table. Per-thread backoff (1 s → 30 s) on empty result.
...
worker:
  ...
  entrypoint: >
    sh -c "sleep 5 && java -cp /app/clp-service.jar
...
| `DB_DATABASE` | `clp_metastore` | Database name |
| `DB_USER` | `root` | Database user |
| `DB_PASSWORD` | `password` | Database password |
| `WORKER_TABLE_NAME` | `clp_spark` | Target metadata table |
| `WORKER_POLL_MIN_BACKOFF_MS` | `1000` | Minimum per-thread backoff (standalone mode only) |
| `WORKER_POLL_MAX_BACKOFF_MS` | `30000` | Maximum per-thread backoff (standalone mode only; in-process prefetcher uses 32s) |
...
|-------|----------|----------|
| No task available | In-process: prefetcher backs off (1s → 32s); workers block in `queue.take()` at zero CPU cost. Standalone: per-thread backoff (1s → 30s) | Automatic |
...
| Planner running? | Check coordinator logs for "PlannerThread" |
