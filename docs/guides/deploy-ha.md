...
| Stall threshold | 5 min | Time before a stalled coordinator triggers a restart (checked during reconciliation) |
...
### Node Liveness vs Stall Detection

Both involve timestamps, but they detect different failures at different layers:

| Layer | Detects | Mechanism | Response |
|-------|---------|-----------|----------|
...
SELECT a.table_name, a.node_id, a.node_assigned_at
...
> **Note:** Stalled coordinator detection is not visible via SQL — it runs in-memory within each node. Check coordinator logs for "coordinator stalled, restarting" messages. A future release will write `last_progress_at` to the database for external monitoring.
