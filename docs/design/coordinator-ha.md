...
Design details, edge cases, walkthroughs, and data model for coordinator high availability. For operational setup and configuration, see [Deploy HA](../guides/deploy-ha.md). For the coordinator's internal thread model, see [Architecture Overview](../concepts/overview.md).
...
4. Claim unassigned active tables via staggered fight-for-master — each node independently tries to claim tables, and the database ensures only one succeeds per table (tables are expected to be pre-registered; if added later, the reconciliation thread picks them up on its next cycle):
   ```
   loop:
...
6. Start background threads:
   ...
   - **Reconciliation thread** — claims unassigned tables, detects dead owners, reconciles running units with DB assignments
   - **Liveness thread** — heartbeat mode writes to `_node_registry`; lease mode renews `lease_expiry`
   - **Watchdog thread** — checks per-thread progress and restarts stalled coordinators (see [Health Monitoring](#health-monitoring))
   ...
   - **Kafka poller** — if enabled, polls Kafka and submits records to the node-level BatchingWriter (see [Ingestion Paths](../concepts/ingestion.md))
   - **Lifecycle threads** — planner, retention, storage deletion
...
If the database is temporarily unreachable, the node logs an error and retries next cycle. A failed liveness write does not stop processing — the liveness thread is independent of per-table coordinators.
...
The liveness thread and watchdog thread cover two non-overlapping failure scenarios:

| Failure | Detected by | Response |
|---------|-------------|----------|
| Node death | Remote peers (liveness thread stops → signal goes stale) | Claim table |
| Coordinator stall (node alive) | Local watchdog | Restart or release |

A stalled coordinator doesn't stop the liveness thread, so remote peers still see the node as alive. Without the watchdog, a stalled coordinator on a live node would never be detected — the liveness signal keeps renewing, no one claims the table, but no work gets done.

Stall detection is entirely local. Each coordinator thread updates an in-memory `lastIterationAt` timestamp at the end of each loop iteration. The watchdog checks these in-memory timestamps — it does not read from the database. If all threads are healthy, the watchdog writes `last_progress_at` to the database for operator visibility (so cluster health can be queried without SSH-ing into individual nodes). If any thread stalls, `last_progress_at` stops advancing.
...
| Step | Trigger | Action |
|------|---------|--------|
| 1 | Thread exceeds stall threshold | Log warning |
| 2 | Thread exceeds 2x stall threshold | Restart per-table coordinator |
| 3 | Restart fails or same coordinator stalls again shortly after | Release assignment (`node_id = NULL`) for another node |

The stall threshold (hardcoded at 50s) is intentionally generous to avoid false positives during transient slowdowns (GC pauses, temporary database latency). The restart threshold is 2x the stall threshold (100s), and the recurrence window is 5 minutes — if the same coordinator stalls again within 5 minutes of a restart, the assignment is released.

**Thread definitions** — a thread has made progress when it completes a full loop iteration:

| Thread | Iteration completes when... |
|--------|---------------------------------|
| Kafka Poller | `poll()` returns and all records submitted to BatchingWriter |
...
An idle thread still completes iterations — `poll()` returns 0 records and finishes immediately. A stuck thread cannot complete its iteration.
...
The local watchdog detects the stall via per-thread `lastIterationAt` timestamps and restarts the per-table coordinator. If restart fails, the node releases the assignment for another node to claim. See [Health Monitoring](#health-monitoring).
...
2. If empty, fall back to `InetAddress.getLocalHost().getHostName()`
...
A node can fail two ways: **node death** (the process crashes) or **coordinator stall** (a thread deadlocks or hangs — the node is alive, but the coordinator is stuck).
...
Design B's simplicity advantage (one fewer table, one fewer thread) is real but bounded. Design A's advantage is that each component has a single responsibility. The system supports both modes — see [Deploy HA](../guides/deploy-ha.md).
