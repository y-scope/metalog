...
The local watchdog detects the stall via `ProgressTracker` (in-memory `lastProgressNanos` atomic timestamp, updated by alias refresh and partition maintenance goroutines) and restarts the per-table coordinator. If restart fails, the node releases the assignment for another node to claim. See [Health Monitoring](#health-monitoring).
