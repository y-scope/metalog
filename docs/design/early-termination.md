...
<details>
<summary><strong>Runnable Go Example (early_termination_demo.go)</strong></summary>

Copy and run this self-contained Go file:

```go
// Early Termination Demo
//
// Run: go run early_termination_demo.go
package main
...
type FileMetadata struct {
	...
	TotalCount     int64
	ErrorPlusCount int64
	FatalPlusCount int64
}

type CountMode int

const (
	Total        CountMode = iota // No filter: use TotalCount
	ErrorOnly                     // level = 'ERROR': use ErrorPlus - FatalPlus
	ErrorAndFatal                 // level >= 'ERROR': use ErrorPlus
)

func getMatchingCount(file FileMetadata, mode CountMode) int64 {
	switch mode {
	case Total:
		return file.TotalCount
	case ErrorOnly:
		return file.ErrorPlusCount - file.FatalPlusCount
	case ErrorAndFatal:
		return file.ErrorPlusCount
	default:
		return file.TotalCount
	}
}

func createSampleFiles() []FileMetadata {
	return []FileMetadata{
		//                   name      created lastMod total error+ fatal+
		{"file01", 300, 450, 60, 25, 8},
		{"file02", 280, 420, 55, 22, 6},
		{"file03", 320, 400, 45, 18, 5},
		{"file04", 290, 380, 50, 20, 4},
		{"file05", 150, 270, 40, 15, 3},
		{"file06", 100, 250, 35, 12, 2},
		{"file07", 80, 200, 30, 10, 2},
		{"file08", 50, 150, 25, 8, 1},
	}
}

func selectFilesWithEarlyTermination(allFiles []FileMetadata, limit int64, mode CountMode) []FileMetadata {
	sorted := make([]FileMetadata, len(allFiles))
	copy(sorted, allFiles)
	sort.Slice(sorted, func(i, j int) bool {
		return sorted[i].LastModifiedAt > sorted[j].LastModifiedAt
	})

	var selected []FileMetadata
	var cumulativeCount int64
	watermark := int64(math.MaxInt64)
	cutoffTs := int64(math.MinInt64)

	for _, file := range sorted {
		matchingCount := getMatchingCount(file, mode)
		...
		if file.CreatedAt < watermark {
			watermark = file.CreatedAt
		}

		if cutoffTs == math.MinInt64 && cumulativeCount >= limit {
			cutoffTs = watermark
		}
	}
...
func runExample(files []FileMetadata, limit int64, mode CountMode) {
	selected := selectFilesWithEarlyTermination(files, limit, mode)

	var totalMatching int64
	for _, f := range selected {
		totalMatching += getMatchingCount(f, mode)
	}

	fmt.Printf("RESULT: %d / %d files selected (%.0f%% pruned), %d events (need %d)\n",
		len(selected), len(files),
		100.0*float64(len(files)-len(selected))/float64(len(files)),
		totalMatching, limit)
}
...
func main() {
	...
	fmt.Println("EXAMPLE 1: No Filter (LIMIT 100)")
	fmt.Println("Matching count = TotalCount")
	runExample(files, 100, Total)

	fmt.Println()
	fmt.Println("EXAMPLE 2: WHERE level = 'ERROR' (LIMIT 20)")
	fmt.Println("Matching count = ErrorPlusCount - FatalPlusCount")
	runExample(files, 20, ErrorOnly)

	fmt.Println()
	fmt.Println("EXAMPLE 3: WHERE level IN ('ERROR', 'FATAL') (LIMIT 50)")
	fmt.Println("Matching count = ErrorPlusCount (cumulative, no subtraction)")
	runExample(files, 50, ErrorAndFatal)
}
```
...
// Good: streams rows on demand, break early
rows, _ := db.QueryContext(ctx, "SELECT ...")
defer rows.Close()
for rows.Next() {
    if done { break }  // Only ~5 rows transferred
}
```

The Go `database/sql` driver streams rows by default when using `rows.Next()`. Combined with keyset pagination, each page fetches only the rows needed.
...
### Streaming
- [ ] Use `rows.Next()` with early break for streaming
- [ ] Keyset pagination for bounded page fetches
