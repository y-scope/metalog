...
2. Delete all tasks except `dead_letter`:
   ...
   ```
3. Ingestion resumes from where it left off
...
|----------|--------------|
| Coordinator crashes mid-operation | On restart: deletes tasks, resumes normal ingestion |
...
The `_task_queue` table is recoverable (gRPC clients re-send on timeout) and can optionally be excluded from replication for high task churn scenarios:
...
**Failover behavior:**
- On restart: delete tasks, ingestion clients reconnect and re-send pending records
...
| Table | Throughput |
|-------|------------|
| Metadata table | ~32K rows/sec (batch-UPSERT) |
...
On restart or failover, the new coordinator deletes in-progress tasks and resumes ingestion from scratch. gRPC clients re-send any unacknowledged records. No offset storage is needed in the database.
...
2. **Coordinator restart:** Kill coordinator → restart → verify task cleanup and ingestion resumption
