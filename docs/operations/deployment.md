...
│                              │   │                              │
│  java -jar clp-service.jar   │   │  java -cp clp-service.jar    │
│    node.yaml                 │   │    c.y.c.s.worker.Worker     │
│                              │   │                              │
...
```bash
java -cp target/clp-service-1.0-SNAPSHOT.jar \
...
```bash
API_GRPC_PORT=9090
DB_JDBC_URL=jdbc:mariadb://replica:3306/clp_metastore
DB_POOL_SIZE=20
```
...
```
jdbc:mariadb://host:3306/clp_metastore
...
services:
  ...
  api-server:
    image: clp-service:latest
    entrypoint: ["java", "-cp", "/app/clp-service.jar",
                 "com.yscope.clp.service.query.api.server.vertx.VertxApiServer"]
    environment:
      API_GRPC_PORT: "9090"
      DB_JDBC_URL: "jdbc:mariadb://mariadb:3306/clp_metastore"
    ports:
      - "9090:9090"
