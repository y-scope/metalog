...
Rough measured characteristics on a local testcontainers-go setup:
...
| Goroutine | Purpose | Throughput Impact |
|-----------|---------|-------------------|
| **Watchdog** | Monitor coordinator goroutine health | Negligible (60s interval) |
...
| More CPU cores | Helps worker goroutine pool |
...
3. **Memory**
   - BatchingWriter channel: bounded per-table capacity
   - Deletion queue: bounded capacity
...
### 1. DSN Configuration (Critical)

**Problem:** The Go MySQL driver must be configured with `interpolateParams=true` for optimal performance. Without it, each parameter requires a separate protocol round-trip for prepared statement binding.

**Solution:** The DSN is built by `DatabaseConfig.DSN()`:

```
user:pass@tcp(host:3306)/db?parseTime=true&interpolateParams=true&maxAllowedPacket=16777216
```

The Go implementation uses `strings.Builder` to construct multi-row INSERT statements directly (not `addBatch()`), so batch performance is inherently good. The `interpolateParams=true` setting avoids server-side prepared statements, reducing round-trips for all queries.
...
The database-backed task queue uses a single atomic `UPDATE ... ORDER BY task_id LIMIT ?` for distributed task claiming. Performance was benchmarked using testcontainers-go (MariaDB).

### Claim Throughput

With the prefetch model, a single `Prefetcher` goroutine issues one DB batch-claim per channel refill regardless of how many worker goroutines are running. DB round-trips scale with task throughput, not with worker count.
...
With prefetch, one `Prefetcher` issues DB claims; worker count no longer multiplies claim IOPS. Frequency below is the **steady-state rate needed** to sustain 10 tasks/sec throughput, not the maximum capacity (which is much higher — see [Claim Throughput](#claim-throughput) above).
