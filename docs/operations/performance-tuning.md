...
**Basic task queue benchmark** — measures task creation, claim, and full lifecycle throughput:
...
Starts a MariaDB container, builds the JAR, and runs `TaskQueueBenchmark` against it.
...
[mysqld]
replicate-ignore-table=clp_metastore._task_queue
```
