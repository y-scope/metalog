...
The CLP Metastore Service is a file-level metadata catalog for CLP's compressed log archives. It coordinates ingestion via gRPC, tracks file lifecycle states, distributes consolidation tasks to workers, and exposes metadata via gRPC for query engines like Presto. By tracking per-file dimensions, aggregates, and time bounds, it enables queries to skip millions of files and terminate early — without opening a single log file.
...
[Extending Metalog](guides/extending.md) → [Architecture Overview](concepts/overview.md) → [Design Documents](design/README.md)
