...
Produces `target/clp-service-1.0-SNAPSHOT.jar` with all dependencies.
...
```
INFO  Node - Starting Node with config: /etc/clp/node.yaml
...
docker compose -f docker/docker-compose.yml exec mariadb mariadb -uroot -ppassword clp_metastore \
  -e "SHOW TABLES;"
