...
CREATE TABLE clp_spark (
    ...
    clp_ir_storage_backend      ENUM('', 'filesystem', 'azure', 'gcs', 'oci', 'r2', 's3', 'minio', 'url', 'tb', 'tb_metadataless') NOT NULL DEFAULT '',
    ...
    clp_archive_storage_backend ENUM('', 'filesystem', 'azure', 'gcs', 'oci', 'r2', 's3', 'minio', 'url', 'tb', 'tb_metadataless') NOT NULL DEFAULT '',
...
| `idx_expiration` | `(expires_at ASC)` | — | Find expired files for retention cleanup | Retention thread (expired files query) |
...
Measured with a single writer thread, `rewriteBatchedStatements=true`, 2 dimension columns, on a single-node database (see [Performance Tuning](../operations/performance-tuning.md) for full methodology and batch size guidance):
...
| Component | Location |
|-----------|----------|
| Schema | `core/src/main/resources/schema/schema.sql` |
| File Records | `core/src/main/java/.../metastore/FileRecords.java` |
| Partition Manager | `core/src/main/java/.../metastore/schema/PartitionManager.java` |
...
**Critical JDBC setting:** `rewriteBatchedStatements=true` required for batch performance (~100x difference).
