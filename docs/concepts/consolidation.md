...
1. **Claim task** via the `TaskPrefetcher`, which batch-claims tasks from `_task_queue` using `SELECT ... FOR UPDATE` + `UPDATE` (READ COMMITTED isolation) and places them in an in-memory queue; worker threads call `take()` on the queue
...
- [Architecture Overview](overview.md) — Thread model, Planner creates consolidation tasks
