...
| **Coordinator** | Stalled goroutine (node alive, goroutine hung) | Local watchdog (`ProgressTracker` — in-memory `lastProgressNanos` atomic timestamp) | Restart coordinator; release assignment if restart fails |
...
4. Create BatchingWriter and IngestionService
5. Start all coordinator units (each runs per-coordinator startup below)
6. Start gRPC server (if enabled)
7. Start node-level goroutines: Heartbeat/Lease Renewal, Reconciliation (includes stall detection)
8. Start Health Check Server (if configured)
