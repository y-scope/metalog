...
func TestExpandDimWidth_RegistryUpdateFails(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	...
	mock.ExpectExec("UPDATE").WillReturnError(fmt.Errorf("update registry failed"))

	_, err := cr.expandDimWidth(context.Background(), entry, 500)
	if err == nil {
		t.Fatal("expected error from registry update failure")
	}
}

func TestExpandDimWidth_ConcurrentInvalidationFixFails(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	...
	// Fix query fails
	mock.ExpectExec("UPDATE").WillReturnError(fmt.Errorf("fix failed"))
...
func TestResolveOrAllocateAgg_CacheHit(t *testing.T) {
	cr := testCR()
	key := AggCacheKey("cpu", "", "SUM")
	cr.aggByKey[key] = &AggRegistryEntry{ColumnName: "agg_f01", AggKey: "cpu", AggValue: "", AggregationType: "SUM", ValueType: "FLOAT"}
	cr.aggByColumn["agg_f01"] = cr.aggByKey[key]

	col, err := cr.ResolveOrAllocateAgg(context.Background(), "cpu", "", "SUM", "FLOAT")
	if err != nil {
		t.Fatal(err)
	}
	if col != "agg_f01" {
...
func TestResolveOrAllocateDim_CacheHit(t *testing.T) {
	cr := testCR()
	cr.dimByKey["host"] = &DimRegistryEntry{ColumnName: "dim_f01", DimKey: "host", BaseType: "str", Width: 256}
	cr.dimByColumn["dim_f01"] = cr.dimByKey["host"]

	col, err := cr.ResolveOrAllocateDim(context.Background(), "host", "str", 128)
	if err != nil {
		t.Fatal(err)
	}
	if col != "dim_f01" {
...
func TestLookupAggFromDB_Error(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	...
	_, err := cr.lookupAggFromDB(context.Background(), "cpu", "", "SUM")
	if err == nil {
		t.Fatal("expected error from lookupAggFromDB")
	}
}

func TestLookupAggFromDB_NotFound(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	...
	col, err := cr.lookupAggFromDB(context.Background(), "missing", "", "SUM")
	if err != nil {
		t.Fatal(err)
	}
	if col != "" {
...
func TestLookupDimFromDB_Found(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	...
	col, err := cr.lookupDimFromDB(context.Background(), "host")
	if err != nil {
		t.Fatal(err)
	}
	if col != "dim_f01" {
