...
func setupColumnRegistryIT(t *testing.T) (*testutil.DBContainer, *schema.ColumnRegistry) {
	t.Helper()
...
func TestColumnRegistry_AllEntries(t *testing.T) {
	...
	if _, err := cr.ResolveOrAllocateDim(ctx, "k1", "str", 64); err != nil {
		t.Fatal(err)
	}
	if _, err := cr.ResolveOrAllocateDim(ctx, "k2", "int", 0); err != nil {
		t.Fatal(err)
	}
	if _, err := cr.ResolveOrAllocateAgg(ctx, "a1", "", "SUM", "INT"); err != nil {
		t.Fatal(err)
	}
...
func TestColumnRegistry_PersistsAcrossReloads(t *testing.T) {
	...
	var allocErr error
	if _, allocErr = cr1.ResolveOrAllocateDim(ctx, "host", "str", 128); allocErr != nil {
		t.Fatal(allocErr)
	}
	if _, allocErr = cr1.ResolveOrAllocateAgg(ctx, "errors", "", "SUM", "INT"); allocErr != nil {
		t.Fatal(allocErr)
	}
...
func TestColumnRegistry_ResolveOrAllocateDims_Batch(t *testing.T) {
	mc, cr := setupColumnRegistryIT(t)
	defer mc.Teardown(t)
	ctx := context.Background()

	reqs := []schema.DimRequest{
		{DimKey: "host", BaseType: "str", Width: 256},
		{DimKey: "region", BaseType: "str", Width: 128},
		{DimKey: "level", BaseType: "int"},
	}
	result, err := cr.ResolveOrAllocateDims(ctx, reqs)
	if err != nil {
		t.Fatalf("ResolveOrAllocateDims: %v", err)
	}
	if len(result) != 3 {
	...
	result2, err := cr.ResolveOrAllocateDims(ctx, reqs)
	if err != nil {
		t.Fatal(err)
	}
	if result2["host"] != result["host"] {
		t.Error("second call should return cached results")
	}
}

func TestColumnRegistry_ResolveOrAllocateAggs_Batch(t *testing.T) {
	mc, cr := setupColumnRegistryIT(t)
	defer mc.Teardown(t)
	ctx := context.Background()

	reqs := []schema.AggRequest{
		{AggKey: "cpu", AggType: "SUM", ValueType: "FLOAT"},
		{AggKey: "mem", AggType: "SUM", ValueType: "INT"},
	}
	result, err := cr.ResolveOrAllocateAggs(ctx, reqs)
	if err != nil {
		t.Fatalf("ResolveOrAllocateAggs: %v", err)
	}
	if len(result) != 2 {
