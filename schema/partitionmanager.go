...
func (pm *PartitionManager) RunMaintenance(ctx context.Context) error {
	...
	defer func() {
		if err := lock.Release(ctx); err != nil {
			pm.log.Warn("release partition maintenance lock failed",
				zap.Error(err))
		}
	}()

	pm.log.Debug("starting partition maintenance")
	...
	if pm.cleanupAgeDays > 0 {
		if err := pm.cleanupOldPartitions(ctx); err != nil {
			pm.log.Warn("partition cleanup failed",
				zap.Error(err))
		}
	}

	pm.log.Debug("partition maintenance completed")
	return nil
}
...
func (pm *PartitionManager) EnsureLookaheadPartitions(ctx context.Context) (int, error) {
	...
	if err != nil {
		...
		defer func() {
			if err := lock.Release(ctx); err != nil {
				pm.log.Warn("release lookahead lock failed",
					zap.Error(err))
			}
		}()
	}
