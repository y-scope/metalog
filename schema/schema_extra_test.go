...
func TestCreatePhysicalTable_ExecError(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	mock.ExpectExec("CREATE TABLE").WillReturnError(fmt.Errorf("exec failed"))

	err := createPhysicalTable(context.Background(), db, "test_table", false, "lz4")
	if err == nil {
		t.Fatal("expected error from exec failure")
	}
}

func TestCreatePhysicalTable_TableExistsIgnored(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	mock.ExpectExec("CREATE TABLE").WillReturnError(&mysql.MySQLError{Number: 1050, Message: "Table already exists"})

	err := createPhysicalTable(context.Background(), db, "test_table", false, "lz4")
	if err != nil {
		t.Fatalf("table-exists error should be ignored, got: %v", err)
	}
}

func TestEnsureTable_CreatePhysicalTableFails(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	mock.ExpectExec("CREATE TABLE").WillReturnError(fmt.Errorf("create failed"))

	err := EnsureTable(context.Background(), db, "test_table", false, "", zap.NewNop())
	if err == nil {
		t.Fatal("expected error from createPhysicalTable failure")
	}
}

func TestEnsureTable_InsertRegistryFails(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	mock.ExpectExec("CREATE TABLE").WillReturnResult(sqlmock.NewResult(0, 0))
	...
	err := EnsureTable(context.Background(), db, "test_table", false, "", zap.NewNop())
	if err == nil {
		t.Fatal("expected error from insertRegistryRows failure")
	}
}

func TestEnsureTable_PrepopulateSketchFails(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	mock.ExpectExec("CREATE TABLE").WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT").WillReturnResult(sqlmock.NewResult(1, 1))
	...
	err := EnsureTable(context.Background(), db, "test_table", false, "", zap.NewNop())
	if err == nil {
		t.Fatal("expected error from prepopulateSketchSlots failure")
	}
}

func TestEnsureTable_CreatePartitionsFails(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	mock.ExpectExec("CREATE TABLE").WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT").WillReturnResult(sqlmock.NewResult(1, 1))
	...
	mock.ExpectQuery("SELECT PARTITION_NAME").WillReturnError(fmt.Errorf("partition query failed"))

	err := EnsureTable(context.Background(), db, "test_table", false, "", zap.NewNop())
	if err == nil {
		t.Fatal("expected error from createLookaheadPartitions failure")
	}
}

func TestEnsureTable_FullSuccess(t *testing.T) {
	db, mock, _ := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	defer db.Close() //nolint:errcheck
	mock.ExpectExec("CREATE TABLE").WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT").WillReturnResult(sqlmock.NewResult(1, 1))
	...
	// createLookaheadPartitions — return partitions with all days covered
	now := time.Now().UTC().Truncate(24 * time.Hour)
	partRows := sqlmock.NewRows([]string{"PARTITION_NAME", "PARTITION_DESCRIPTION", "TABLE_ROWS", "DATA_LENGTH"})
	partRows.AddRow("p_floor", sql.NullString{}, int64(0), int64(0))
	for i := 0; i <= defaultProvisionLookaheadDays; i++ {
		d := now.AddDate(0, 0, i)
		name := fmt.Sprintf("p_%s", d.Format("20060102"))
		boundary := d.AddDate(0, 0, 1).UnixNano()
		partRows.AddRow(name, sql.NullString{Valid: true, String: fmt.Sprintf("%d", boundary)}, int64(0), int64(0))
	}
	partRows.AddRow("p_future", sql.NullString{Valid: true, String: "MAXVALUE"}, int64(0), int64(0))
	mock.ExpectQuery("SELECT PARTITION_NAME").WillReturnRows(partRows)
