...
// If leaseTTL > 0, sets lease_expiry on the claimed row (lease mode).
// Pass 0 for heartbeat mode (no lease needed).
func (r *CoordinatorRegistry) ClaimTable(ctx context.Context, tableName string, leaseTTL time.Duration) (bool, error) {
...
// CountActiveNodesHeartbeat returns the number of nodes with a recent heartbeat.
func (r *CoordinatorRegistry) CountActiveNodesHeartbeat(ctx context.Context, deadThreshold time.Duration) (int, error) {
	cutoff := time.Now().Add(-deadThreshold).UnixNano()
...
// CountActiveNodesLease returns the number of distinct nodes with a valid lease.
func (r *CoordinatorRegistry) CountActiveNodesLease(ctx context.Context) (int, error) {
	now := time.Now().UnixNano()
