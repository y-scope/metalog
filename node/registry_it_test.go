...
func setupRegistryIT(t *testing.T) (*testutil.DBContainer, *registry.Registry) {
	t.Helper()
...
// registerTestTable inserts a table into the registry and assignment tables.
// Tables are now registered via admin API in production.
...
func TestRegistry_GetAssignedTables(t *testing.T) {
	...
	cr.ClaimTable(ctx, "assigned_a", 0)  //nolint:errcheck // test setup
	cr.ClaimTable(ctx, "assigned_b", 0)  //nolint:errcheck // test setup
...
func TestRegistry_ReleaseAllTables(t *testing.T) {
	...
	cr.ClaimTable(ctx, "release_all_a", 0) //nolint:errcheck // test setup
	cr.ClaimTable(ctx, "release_all_b", 0) //nolint:errcheck // test setup
...
func TestRegistry_ClaimByDifferentNodes(t *testing.T) {
	...
	// Node 1 releases
	node1.ReleaseTable(ctx, "contested") //nolint:errcheck // test setup
...
func TestRegistry_ClaimOrphansFromDeadNodes(t *testing.T) {
	...
	node1.SendHeartbeat(ctx)               //nolint:errcheck // test setup
	node1.ClaimTable(ctx, "orphan_table", 0) //nolint:errcheck // test setup

	// Set dead-node's heartbeat to the past (simulate death)
	pastNanos := time.Now().Add(-300 * time.Second).UnixNano()
	mc.DB.ExecContext(ctx, //nolint:errcheck // test setup
		"UPDATE _node_registry SET last_heartbeat_at = ? WHERE node_id = ?",
		pastNanos, "dead-node")

	// Alive node sends heartbeat and claims orphans
	node2.SendHeartbeat(ctx) //nolint:errcheck // test setup
