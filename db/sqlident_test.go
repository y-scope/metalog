...
func TestQuoteIdentifier_Valid(t *testing.T) {
	got := QuoteIdentifier("my_table")
	if got != "`my_table`" {
		t.Errorf("QuoteIdentifier(my_table) = %q, want `my_table`", got)
	}
}

func TestQuoteIdentifier_PanicsOnInvalid(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Error("QuoteIdentifier should panic for invalid identifier")
		}
	}()
	QuoteIdentifier("INVALID-NAME!")
}
