...
func Marshal(v any) ([]byte, error) {
	...
	if err := compressLZ4(raw, &buf); err != nil {
		return nil, err
	}
	return buf.Bytes(), nil
}

// compressLZ4 writes raw bytes through an LZ4 compressor to dst.
func compressLZ4(raw []byte, dst io.Writer) error {
	w := lz4.NewWriter(dst)
	if _, err := w.Write(raw); err != nil {
		return fmt.Errorf("lz4 write: %w", err)
	}
