...
Metalog can be embedded as a library inside a host application (e.g., a Spring service). All application code uses SLF4J for logging; Log4j2 is included as the default binding for standalone deployment.

### Logging

To use a different SLF4J binding (e.g., Logback), exclude the bundled Log4j2 dependencies and provide your own:

```xml
<dependency>
    <groupId>com.yscope</groupId>
    <artifactId>metalog-core</artifactId>
    <version>${metalog.version}</version>
    <exclusions>
        <exclusion>
...
The host application's SLF4J binding (Logback, Log4j2, etc.) will be used automatically.

### gRPC server

The `metalog-core` module contains only proto definitions and generated stubs (`grpc-protobuf`, `grpc-stub`) — no server runtime. The bundled gRPC server (Netty-based, in `metalog-grpc-server`) is a reference implementation.

If the host application has its own gRPC server, depend on `metalog-core` directly and register the service implementations on your server. The generated `*Grpc.*.bindService()` methods work with any gRPC server runtime (Netty, Servlet, in-process, etc.).
