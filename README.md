...
- **High-throughput mutable metadata** — unlike traditional metastores that treat metadata as immutable snapshots, CLP's file metadata is continuously updated: state, counts, and dimensions change as files progress through their lifecycle. The metastore sustains 10K+ upserts per second per ingestion goroutine, scales to billions of files per table and dozens of tables per database, with a small operating footprint (e.g., 4 cores, 32 GB RAM per node, or a handful of these nodes with HA enabled).
...
The service has two roles: **coordinator nodes** that ingest metadata and manage table lifecycles, and **stateless workers** that execute consolidation tasks. MariaDB/MySQL is the single source of truth — coordinators write metadata, workers claim tasks, and both read from it. In production, the roles run as separate processes; for development and testing, they colocate in a single Node process.
...
- **Admin API table registration** — register tables at runtime via gRPC admin API; coordinators discover assignments from the database
...
<tr><td rowspan="4">Operations</td><td><a href="docs/operations/performance-tuning.md">Performance Tuning</a></td><td>Benchmarks, SQL driver gotchas, index strategy</td></tr>
...
Configuration is split between `node.yaml` (node-level settings) and the database (per-table config). Tables are registered via the admin gRPC API (`metalog admin register-table`); coordinators discover assigned tables from the database on startup and via periodic reconciliation. See [Configuration Reference](docs/reference/configuration.md) for the full reference.
