...
func (m *mockArchiveCreator) DeleteArchive(_ context.Context, _, _, _ string) error {
	m.deleteCalls++
	return nil
}
...
func TestExecuteTask_ArchiveCreationFailure(t *testing.T) {
	...
	// Verify orphan archive cleanup was attempted.
	if ac.deleteCalls != 1 {
		t.Errorf("DeleteArchive called %d times, want 1", ac.deleteCalls)
	}
}
