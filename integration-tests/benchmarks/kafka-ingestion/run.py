#!/usr/bin/env python3
"""
End-to-end Kafka ingestion throughput benchmark for the Metalog coordinator.
...
def main():
    ...
    print()
    print("==========================================")
    print("  Kafka Ingestion Benchmark")
    print("==========================================")
    print()
    print("Configuration:")
    ...
    if not args.skip_infra:
        ...
        r = subprocess.run(dc + ["down", "-v", "--remove-orphans"], capture_output=True, text=True)
        if r.returncode != 0:
            log_warn(f"docker compose down failed: {r.stderr.strip()}")
        r = subprocess.run(
            ["docker", "rm", "-f", "metalog-mariadb", "metalog-kafka", "metalog-zookeeper",
             "metalog-minio", "metalog-minio-init", "metalog-coordinator",
             "metalog-mariadb-task-queue", "metalog-mariadb-scalability"],
            capture_output=True, text=True,
        )
        if r.returncode != 0:
            log_warn(f"docker rm failed: {r.stderr.strip()}")
        log_success("Containers cleaned")
    else:
        subprocess.run(["docker", "rm", "-f", "metalog-coordinator"], capture_output=True)
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", prefix="metalog-kafka-benchmark-", suffix=".yaml", delete=False
    ) as f:
        _temp_config = f.name
        f.write(f"""\
node:
