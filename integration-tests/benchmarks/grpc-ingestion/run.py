#!/usr/bin/env python3
"""
End-to-end gRPC ingestion throughput benchmark for the CLP Service coordinator.
...
def main():
    ...
    if not args.skip_infra:
        ...
        subprocess.run(
            ["docker", "rm", "-f", "clp-mariadb", "clp-mariadb-task-queue", "clp-mariadb-scalability"],
            capture_output=True,
        )
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", prefix="clp-grpc-benchmark-", suffix=".yaml", delete=False
    ) as f:
        _temp_config = f.name
        f.write(f"""\
node:
