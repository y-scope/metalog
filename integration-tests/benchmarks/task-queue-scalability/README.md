...
- **Claim throughput** (tasks/sec) — `SELECT ... FOR UPDATE` + `UPDATE` in a READ COMMITTED transaction
...
Results are printed as a table with one row per worker count:

```
CLAIM THROUGHPUT (tasks/sec):
Workers          Batch=1          Batch=5         Batch=10
-------          -------          -------         --------
      1               55              290              520
     25              210              980             1800
     50              310             1250             2100
...
Throughput typically peaks around 50–100 workers for batch=1 (lock contention increases
beyond that). Larger batch sizes shift the peak higher by reducing transaction overhead.
With 10 tables (default), workers spread across tables and contention is minimal.

See [Performance Tuning](../../../docs/operations/performance-tuning.md#task-queue-performance)
for detailed analysis of the bottlenecks.
