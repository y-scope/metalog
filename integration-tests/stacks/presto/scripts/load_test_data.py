...
def connect(deadline=60):
    ...
    # Codes that mean the server isn't up yet
    _NOT_READY = {grpc.StatusCode.UNAVAILABLE, grpc.StatusCode.DEADLINE_EXCEEDED}

    start = time.time()
    while True:
        try:
            probe = ingestion_pb2.IngestRequest(table_name='__probe__',
                                                record=ingestion_pb2.MetadataRecord())
            stub.Ingest(probe, timeout=2)
            logger.info('Connected to coordinator at %s', target)
            return stub
        except grpc.RpcError as e:
            if e.code() not in _NOT_READY:
                # Server is up — any business-logic error means it's accepting connections
                logger.info('Connected to coordinator at %s', target)
                return stub
