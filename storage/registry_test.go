...
func TestRegisterType(t *testing.T) {
	RegisterType("test_backend", BackendMeta{
		Factory:        func(m map[string]string) (Backend, error) { return nil, nil },
		RequiresBucket: true,
	})
	b, err := CreateBackend("test_backend", nil)
	if err != nil {
		t.Fatal(err)
	}
	if b != nil {
		t.Error("expected nil backend from factory")
	}
	if !RequiresBucket("test_backend") {
		t.Error("test_backend should require bucket")
	}
}
...
func TestRequiresBucket_UnknownType(t *testing.T) {
	if !RequiresBucket("nonexistent") {
