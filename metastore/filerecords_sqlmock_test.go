...
func TestAdvisoryLock_AcquireQueryError(t *testing.T) {
	db, mock, _ := sqlmock.New()
	...
	_, err := AcquireAdvisoryLock(context.Background(), db, "test_lock", 0)
	if err == nil {
		t.Fatal("expected error from GET_LOCK query failure")
	}
}

func TestAdvisoryLock_AcquireNullResult(t *testing.T) {
	db, mock, _ := sqlmock.New()
	defer db.Close() //nolint:errcheck
	...
	_, err := AcquireAdvisoryLock(context.Background(), db, "test_lock", 0)
	if err == nil {
		t.Fatal("expected error for NULL result from GET_LOCK")
	}
}

func TestAdvisoryLock_ReleaseQueryError(t *testing.T) {
	db, mock, _ := sqlmock.New()
	defer db.Close() //nolint:errcheck
	...
	lock, err := AcquireAdvisoryLock(context.Background(), db, "lock1", 0)
	if err != nil {
		t.Fatalf("AcquireAdvisoryLock() error = %v", err)
	}
	...
	err = lock.Release(context.Background())
	if err == nil {
		t.Fatal("expected error from RELEASE_LOCK failure")
	}
	// Lock should be marked as not held even on error.
	if lock.held {
		t.Error("lock.held should be false after Release")
	}
}
...
func TestNullableBytes(t *testing.T) {
	...
	if got := nullableBytes([]byte{}); got != nil {
		t.Errorf("nullableBytes([]) = %v, want nil", got)
	}
	data := []byte{1, 2, 3}
	if got := nullableBytes(data); got == nil {
		t.Errorf("nullableBytes(data) = nil, want non-nil")
	}
}

func TestUpdateState_MissingFileSkipped(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)

	mock.ExpectBegin()
	// getCurrentStatesInTx returns only /ir/2 — /ir/1 is missing
	stateRows := sqlmock.NewRows([]string{"clp_ir_path", "state"}).
		AddRow("/ir/2", "IR_BUFFERING")
	mock.ExpectQuery("SELECT").WillReturnRows(stateRows)
	mock.ExpectExec("UPDATE").WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()

	affected, err := fr.UpdateState(context.Background(), []string{"/ir/1", "/ir/2"}, StateIRClosed)
	if err != nil {
		t.Fatalf("error = %v", err)
	}
	if affected != 1 {
...
func TestUpdateState_AllMissingCommits(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	...
	affected, err := fr.UpdateState(context.Background(), []string{"/ir/missing"}, StateIRClosed)
	if err != nil {
		t.Fatalf("error = %v", err)
	}
	if affected != 0 {
...
func TestFindConsolidationPending_QueryError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectQuery("SELECT").WillReturnError(sql.ErrConnDone)

	_, err := fr.FindConsolidationPending(context.Background(), nil, nil)
	if err == nil {
		t.Fatal("expected error")
	}
}

func TestGetCurrentStates_QueryError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectQuery("SELECT").WillReturnError(sql.ErrConnDone)

	_, err := fr.GetCurrentStates(context.Background(), []string{"/ir/1"})
	if err == nil {
		t.Fatal("expected error")
	}
}

func TestPromoteStuckBuffering_ExecError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectExec("UPDATE").WillReturnError(sql.ErrConnDone)

	_, err := fr.PromoteStuckBuffering(context.Background(), 1000000)
	if err == nil {
		t.Fatal("expected error")
	}
}

func TestTransitionExpiredToPurging_FirstFails(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectExec("UPDATE").WillReturnError(sql.ErrConnDone)

	_, err := fr.TransitionExpiredToPurging(context.Background(), 9999999)
	if err == nil {
		t.Fatal("expected error from first transition")
	}
}

func TestTransitionExpiredToPurging_SecondFails(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectExec("UPDATE").WillReturnResult(sqlmock.NewResult(0, 2))
	mock.ExpectExec("UPDATE").WillReturnError(sql.ErrConnDone)
	...
	_, err := fr.TransitionExpiredToPurging(context.Background(), 9999999)
	if err == nil {
		t.Fatal("expected error from second transition")
	}
}

func TestUpsertBatch_ExecError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectExec("INSERT INTO").WillReturnError(sql.ErrConnDone)

	records := []*FileRecord{{State: StateIRBuffering, ClpIRPath: sql.NullString{Valid: true, String: "/ir/p"}}}
	_, err := fr.UpsertBatch(context.Background(), records, nil, nil, nil)
	if err == nil {
		t.Fatal("expected error")
	}
}
...
func TestFindByID_QueryError(t *testing.T) {
	...
	_, err := fr.FindByID(context.Background(), 1)
	if err == nil {
		t.Fatal("expected error")
	}
}

func TestFindByIRPath_QueryError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectQuery("SELECT").WillReturnError(sql.ErrConnDone)

	_, err := fr.FindByIRPath(context.Background(), "/ir/1")
	if err == nil {
		t.Fatal("expected error")
	}
}

func TestFindByArchivePath_QueryError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectQuery("SELECT").WillReturnError(sql.ErrConnDone)

	_, err := fr.FindByArchivePath(context.Background(), "/archive/1")
	if err == nil {
		t.Fatal("expected error")
	}
}

func TestMarkArchiveClosed_BeginTxError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
...
func TestUpdateState_BeginTxError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectBegin().WillReturnError(sql.ErrConnDone)
...
func TestDeleteExpiredFiles_BeginTxError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	mock.ExpectBegin().WillReturnError(sql.ErrConnDone)

	_, err := fr.DeleteExpiredFiles(context.Background(), 9999999)
	if err == nil {
		t.Fatal("expected error from BeginTx")
	}
}

func TestMarkArchiveClosed_ExecError(t *testing.T) {
	fr, mock := newMockFileRecords(t, "test_table", true)
	...
	_, err := fr.MarkArchiveClosed(context.Background(),
		[]string{"/ir/1"}, "/archive/path", "s3", "bucket", 5000, 1234567)
	if err == nil {
