...
// insertTestRecord inserts a file record into the test table.
func insertTestRecord(t *testing.T, db *sql.DB, minTS, maxTS int64, irPath, state string, expiresAt ...int64) {
	t.Helper()
	exp := int64(0)
	if len(expiresAt) > 0 {
		exp = expiresAt[0]
	...
	query, args, err := sq.Insert("`"+testTable+"`").
		...
		Values(minTS, maxTS, irPath, state, 10, 30, exp).
		ToSql()
	if err != nil {
...
func setupFileRecordsIT(t *testing.T) (*testutil.DBContainer, *metastore.FileRecords) {
	t.Helper()
...
func TestFileRecords_UpsertBatch_GuardPreventsOverwrite(t *testing.T) {
	...
	if _, execErr := mc.DB.ExecContext(ctx, query, args...); execErr != nil {
		t.Fatal(execErr)
	}
