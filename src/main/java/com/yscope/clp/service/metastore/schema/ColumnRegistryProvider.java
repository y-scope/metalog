...
public class ColumnRegistryProvider {
  ...
  public ColumnRegistryProvider(DataSource dataSource) {
    this(dataSource, DEFAULT_TTL_MS, FAILURE_TTL_MS);
  }

  public ColumnRegistryProvider(DataSource dataSource, long ttlMs) {
    this(dataSource, ttlMs, FAILURE_TTL_MS);
  }

  public ColumnRegistryProvider(DataSource dataSource, long ttlMs, long failureTtlMs) {
    ...
    CachedRegistry entry =
        cache.compute(
            ...
            (key, cached) -> {
              ...
              try {
                ColumnRegistry registry = new ColumnRegistry(dataSource, key);
