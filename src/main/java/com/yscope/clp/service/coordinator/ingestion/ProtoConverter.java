...
public final class ProtoConverter {
  ...
  public static FileRecord toFileRecord(MetadataRecord record) {
    ...
    for (AggEntry agg : record.getAggList()) {
      ...
      switch (agg.getValueCase()) {
        case INT_VAL -> file.setCount(compositeKey, agg.getIntVal());
        case FLOAT_VAL -> file.setFloatCount(compositeKey, agg.getFloatVal());
  ...
  static void parseSelfDescAgg(String key, String rawValue, FileRecord file) {
    ...
    try {
      if (isFloat) {
        file.setFloatCount(compositeKey, Double.parseDouble(rawValue));
      } else {
        file.setCount(compositeKey, Long.parseLong(rawValue));
      }
