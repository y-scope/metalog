...
public class FilterExpressionValidator {
  ...
  private static void validateExpression(Expression expr) {
    if (expr instanceof AndExpression and) {
      ...
      validateExpression(parenthesis.getExpression());
      ...
      // JSqlParser 4.3: subqueries go to getRightExpression(), lists go to getRightItemsList()
      ...
      } else if (rightItems instanceof ExpressionList) {
        for (Expression child : ((ExpressionList) rightItems).getExpressions()) {
          validateExpression(child);
        }
      } else if (rightItems != null) {
        throw new IllegalArgumentException(
            "Unsupported IN right-hand side type: " + rightItems.getClass().getSimpleName());
      }
