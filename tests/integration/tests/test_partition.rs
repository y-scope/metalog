...
mod tests {
    ...
    async fn lookahead_creates_seven_daily_partitions() {
        ...
        let mgr = PartitionManager::new(pool.clone(), "test_part_lookahead", 7, 90);
    ...
    async fn lookahead_is_idempotent() {
        ...
        let mgr = PartitionManager::new(pool.clone(), "test_part_idem", 7, 90);
    ...
    async fn custom_lookahead_count_respected() {
        ...
        let mgr = PartitionManager::new(pool.clone(), "test_part_custom", 3, 90);
        let created = mgr.ensure_lookahead_partitions().await.unwrap();
    ...
    async fn run_maintenance_succeeds_and_creates_partitions() {
        ...
        let mgr = PartitionManager::new(pool.clone(), "test_part_maint", 7, 90);
    ...
    async fn run_maintenance_twice_is_idempotent() {
        ...
        let mgr = PartitionManager::new(pool.clone(), "test_part_maint2", 7, 90);
        mgr.run_maintenance().await.unwrap();
        mgr.run_maintenance().await.unwrap();
