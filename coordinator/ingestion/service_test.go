...
func TestIngestWithCallbackWait_SetsFlushedChan(t *testing.T) {
	bw := newTestWriter(t)
	s := NewService(bw, false, zap.NewNop())
	flushed := make(chan error, 1)
	rec := &metastore.FileRecord{}
	if err := s.IngestWithCallbackWait(context.Background(), "my_table", rec, flushed); err != nil {
		t.Fatalf("IngestWithCallbackWait error: %v", err)
	}
	if rec.Flushed != flushed {
		t.Error("rec.Flushed not set to provided channel")
	}
...
func TestIngestWithCallbackWait_EmptyTableName(t *testing.T) {
	bw := newTestWriter(t)
	s := NewService(bw, false, zap.NewNop())
	err := s.IngestWithCallbackWait(context.Background(), "", &metastore.FileRecord{}, nil)
	if err == nil {
		t.Error("expected error for empty table name")
	}
}
