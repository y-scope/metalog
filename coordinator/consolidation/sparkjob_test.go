...
func TestSparkJobConfig_CustomValues(t *testing.T) {
	cfg := sparkJobConfig{
		GroupingDimKey: "app",
		MinFiles:       5,
		MaxFiles:       50,
	...
	if got := cfg.minFiles(); got != 5 {
		t.Errorf("minFiles = %d, want 5", got)
	}
	if got := cfg.maxFiles(); got != 50 {
		t.Errorf("maxFiles = %d, want 50", got)
	}
	if got := cfg.jobTimeout(); got != 2*time.Hour {
		t.Errorf("jobTimeout = %v, want 2h", got)
	}
}

func TestSparkJobConfig_InvalidTimeout(t *testing.T) {
	cfg := sparkJobConfig{JobTimeout: "invalid"}
	if got := cfg.jobTimeout(); got != defaultJobTimeout {
		t.Errorf("jobTimeout for invalid = %v, want %v", got, defaultJobTimeout)
	}
}
