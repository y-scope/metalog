...
func (c *timeWindowConfig) windowSize() (time.Duration, error) {
	if c.WindowSize != "" {
		d, err := time.ParseDuration(c.WindowSize)
		if err != nil {
			return 0, fmt.Errorf("invalid window_size %q: %w", c.WindowSize, err)
		}
		if d <= 0 {
			return 0, fmt.Errorf("window_size must be positive, got %q", c.WindowSize)
		}
		return d, nil
	}
