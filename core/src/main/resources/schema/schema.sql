...
--     (e.g., IR_BUFFERING can only go to IR_CLOSED) are enforced in FileState.java,
...
--     All state transitions go through FileRecords.java, which uses guarded UPSERTs
...
-- Column registry: ColumnRegistry.java (placeholder name mapping)
-- Partition management: PartitionManager.java (lifecycle automation)
--
...
CREATE TABLE IF NOT EXISTS _clp_template (
    ...
    --
    -- See ColumnRegistry.java for automatic column creation and
...
CREATE TABLE IF NOT EXISTS _sketch_registry (
    table_name      VARCHAR(64) NOT NULL,
    set_member      VARCHAR(8) NOT NULL,
    sketch_key      VARCHAR(1024) NULL,
    status          ENUM('ACTIVE','INVALIDATED','AVAILABLE') NOT NULL,
    created_at      INT UNSIGNED NOT NULL DEFAULT (UNIX_TIMESTAMP()),
    invalidated_at  INT UNSIGNED NULL,
    PRIMARY KEY (table_name, set_member),
    FOREIGN KEY (table_name) REFERENCES _table(table_name)
) ENGINE=InnoDB;
